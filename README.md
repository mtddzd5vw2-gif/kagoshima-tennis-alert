...
> [!IMPORTANT]
> 鴨池県営テニスコートは実DOMに対応済みです。SuMIzeiはURLと実DOMが未確定のため `selector_pending` です。自動予約、ログイン、利用者ID・パスワードの使用や保存は行いません。
...
実行時には `snapshots/kamoike-prefectural/YYYY-MM-DD.html` と同名のPNGを生成します。スナップショットはGit管理せず、GitHub ActionsのArtifactとして7日間保存します。
...
- `selector_pending`: 未実装施設

エラー時も `checked_at`、`reservation_url`、空の `availability` を保存します。主な `error_type` は `navigation_timeout`、`navigation_error`、`access_denied`、`no_schedule_table`、`unexpected_dom` です。
...
SuMIzeiは実DOM調査前です。今後、次のGitHub Actions Variablesを設定して施設固有パーサーを実装します。

| Variable | 用途 |
| --- | --- |
| `SUMIZEI_URL_TEMPLATE` | `{date}` を含む日別予約URLテンプレート |
| `SUMIZEI_SLOT_SELECTOR` | 空き枠候補を絞り込むPlaywrightセレクタ |
...
1. SuMIzeiの正式URLと実DOMを調査する
2. 取得失敗時のLINEエラー通知方針を追加する
3. GitHub Actionsの外部ActionをコミットSHAで固定する
4. サイト利用規約と適切なアクセス頻度を継続確認する
