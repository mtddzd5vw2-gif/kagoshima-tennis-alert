...
- 会員基盤の候補はSupabase Auth/PostgreSQLとする。正式採用、構成、リージョン、料金枠は**要決定**。
...
| Phase 1 | 次に着手 | 規約同意・メール認証を伴う会員登録、ログイン、マイページを提供する |
| Phase 2 | 計画 | 利用者が通知条件を保存・変更できる |
...
- Supabase Auth/PostgreSQLを候補とした技術検証結果と採否記録
- 開発・ステージング・本番の環境分離方針
...
- Supabaseの正式採用可否が決定している。採用しない場合は代替案が記録されている。
...
- メール送信元、独自ドメイン、リンク有効期限、配信事業者は**要決定**として残さず、リリース前に決定されている。
...
- 正しい認証情報とメール認証済み状態でのみログインできる。
- 未認証、無効化、退会済みの会員は保護画面へアクセスできない。
- ログアウト後に保護画面へ戻れない。
- セッション情報をURL、ログ、GitHub管理ファイルへ出力しない。
- パスワード再設定をPhase 1に含めるかが決定され、含める場合は一連のフローをテスト済みである。
