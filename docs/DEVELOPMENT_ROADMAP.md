...
| Phase 2 | 次に着手 | 利用者が通知条件を保存・編集・停止できる |
...
- [Phase 2 通知条件データモデル設計](./PHASE2_NOTIFICATION_RULES_DESIGN.md)と静的スキーマテストを追加した。
- Phase 2の状態は引き続き「次に着手」である。通知条件UI、条件数上限、空き候補との照合ロジックは未完了である。
